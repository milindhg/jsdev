...
will really help.
Step wise simply filter based on input, then sort based on rating and then map
to give only ids in the answer. Easy Peasy.
...
var filterRestaurants = function(restaurants, veganFriendly, maxPrice, maxDistance) {
    return restaurants.filter( r => {
        return ((veganFriendly ? r[2]==veganFriendly : true) && r[3]<=maxPrice && r[4]<=maxDistance);
    }).sort((r1, r2) => r2[1]==r1[1] ? r2[0] - r1[0] : r2[1] - r1[1]).map(r => r[0]);
};
// @lc code=end
...
let main = () => {
    ...
    console.log(filterRestaurants([[1,4,1,40,10],[2,8,0,50,5],[3,8,1,30,4],[4,10,0,10,3],[5,1,1,15,1]], 0, 30, 3));
};
